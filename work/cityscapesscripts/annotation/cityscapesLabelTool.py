...
class configuration:
    ...
    def fixConsistency(self):
        ...
        if not os.path.isfile(self.currentFile) or os.path.dirname(
                self.currentFile) != self.city:
            self.currentFile = ""
...
class CityscapesLabelTool(QtGui.QMainWindow):
    ...
    def initUI(self):
        ...
        clearPolAction = QtGui.QAction(
            QtGui.QIcon(os.path.join(iconDir, 'clearpolygon.png')), '&Tools',
            self)
        ...
        deleteObjectAction = QtGui.QAction(
        ...
        undoAction = QtGui.QAction(
            QtGui.QIcon(os.path.join(iconDir, 'undo.png')), '&Tools', self)
        ...
        highlightAction = QtGui.QAction(
            QtGui.QIcon(os.path.join(iconDir, 'highlight.png')), '&Tools',
            self)
        ...
        # Take a screenshot
        screenshotAction = QtGui.QAction(
            QtGui.QIcon(os.path.join(iconDir, 'screenshot.png')), '&Tools',
            self)
        ...
        screenshotToggleAction = QtGui.QAction(
            QtGui.QIcon(os.path.join(iconDir, 'screenshotToggle.png')),
            '&Tools', self)
        ...
        self.correctAction = QtGui.QAction(
            QtGui.QIcon(os.path.join(iconDir, 'checked6.png')), '&Tools', self)
        ...
        if self.config.correctionMode:
            self.correctAction.setIcon(
                QtGui.QIcon(os.path.join(iconDir, 'checked6_red.png')))
        ...
        for gt in annotations:
            for split in splits:
                ...
                availableCities.extend([(split, gt, os.path.basename(c))
                                        for c in cities if os.path.isdir(c)])
    ...
    def toggleCorrectionMode(self):
        if not self.config.correctionMode:
            ...
            iconDir = os.path.join(os.path.dirname(sys.argv[0]), 'icons')
            self.correctAction.setIcon(
                QtGui.QIcon(os.path.join(iconDir, 'checked6.png')))
        self.update()
        return
    ...
    def save(self):
        ...
        if self.changes and (self.annotation or self.corrections
        ) and self.config.currentFile and self.image:
            if self.annotation:
                ...
                if filename:
                    ...
                    if os.path.isfile(
                            filename) and self.config.showSaveWarning:
    ...
    def undo(self):
        ...
        if len(self.changes) > 1:
            ...
            ret = QtGui.QMessageBox.question(self, dlgTitle, text, buttons,
                                             QtGui.QMessageBox.Ok)
            proceed = False
    ...
    def deleteObject(self):
        ...
        # Save changes
        self.addChange("Deleted object {0} with label {1}".format(
            obj.id, obj.label))
    ...
    def modifyLabel(self):
        ...
        if ok and label:
            for selObj in self.selObjs:
                ...
                if obj.label != label:
                    self.addChange(
                        "Set label {0} for object {1} with previous label {2}".
                            format(label, obj.id, obj.label))
                    obj.label = label
                    obj.updateDate()
    ...
    def screenshot(self):
        ...
        answer = QtGui.QFileDialog.getSaveFileName(
            self,
            dlgTitle,
            self.config.screenshotFilename,
            filter,
    ...
    def loadCity(self):
        ...
        if os.path.isdir(self.config.city):
            self.images = glob.glob(
                os.path.join(self.config.city, '*' + self.imageExt))
            self.images.sort()
    ...
    def modify_correction_description(self):
        if self.selected_correction >= 0 and self.config.correctionMode:
            ...
            if description[1]:
                self.corrections[
                    self.selected_correction].annotation = description[0]
    ...
    def getHighlightedObject(self, qp):
        ...
        if not highlightObjIds and (
                self.drawPoly.isEmpty() or self.drawPolyClosed
        ) and self.mouseObj >= 0 and not self.mouseOutsideImage:
            highlightObjIds = [self.mouseObj]
        # Get the actual object that is highlighted
        if highlightObjIds:
            self.highlightObjs = [
                self.annotation.objects[i] for i in highlightObjIds
            ]
    ...
    def drawLabels(self, qp, ignore=[]):
        ...
        for obj in self.highlightObjs:
            ...
            polyToDraw = self.getPolygon(obj) * QtGui.QTransform.fromScale(
                self.scale, self.scale)
            qp2.drawPolygon(polyToDraw)
    ...
    def drawDrawPoly(self, qp, transform=None):
        ...
        if self.mousePosScaled is not None:
            closestPt = self.getClosestPoint(self.drawPoly,
                                             self.mousePosScaled)
        else:
            closestPt = (-1, -1)
        ...
        if not self.drawPolyClosed:
            # Draw
            self.drawPoint(qp, poly.first(), True, closestPt == (0, 0)
                           and self.drawPoly.size() > 1)
            # Do not draw again
            startDrawingPts = 1
        
        # The next in red
        for pt in range(startDrawingPts, poly.size()):
            self.drawPoint(qp, poly[pt], False, self.drawPolyClosed
                           and closestPt == (pt, pt))
    ...
    def drawLabelAtMouse(self, qp):
        ...
        # Will show zoom
        showZoom = self.config.zoom and not self.image.isNull(
        ) and self.w and self.h
            ...
            if self.draggedPt >= 0:
                ...
                if self.selObjs:
                    ...
                    if not obj.id in self.changedPolygon:
                        ...
                        self.addChange(
                            "Changed polygon of object {0} with label {1}".
                                format(obj.id, obj.label))
        else:
            if self.in_progress_bbox is not None:
    ...
    def mousePressEvent(self, event):
        ...
        shiftPressed = QtGui.QApplication.keyboardModifiers(
        ...
        if event.button() == QtCore.Qt.LeftButton:
            ...
            if not self.config.correctionMode:
                if self.drawPolyClosed and (self.mousePosScaled is not None):
                    ...
                    if shiftPressed:
                        ...
                        # If we got an edge, we insert a point and make it dragged
                            ...
                            self.drawPoly.insert(closestPt[1],
                                                 self.mousePosScaled)
                            ...
                            if self.selObjs:
                                ...
                                obj.polygon.insert(
                                    closestPt[1],
                                    Point(self.mousePosScaled.x(),
                                          self.mousePosScaled.y()))
                                # Check if we changed the object's polygon the first time
                                if not obj.id in self.changedPolygon:
                                    self.changedPolygon.append(obj.id)
                                    self.addChange(
                ...
                self.in_progress_bbox = QtCore.QRectF(self.mousePosScaled.x(),
                                                      self.mousePosScaled.y(),
                                                      0, 0)
                    ...
                    if self.in_progress_bbox.width() > 20:
                        ...
                        if description[1] and description[0]:
                            self.corrections.append(
                                CorrectionBox(self.in_progress_bbox,
                                              annotation=description[0]))
                            # last_annotation = self.in_progress_annotation  #TODO: self?
                            self.corrections[
                                self.selected_correction].unselect()
                            self.selected_correction = len(
                                self.corrections) - 1
                            self.corrections[self.selected_correction].select()
    ...
    def updateMousePos(self, mousePosOrig):
        if self.config.zoomFactor <= 1 or (self.drawPolyClosed
                                           or self.drawPoly.isEmpty()):
            sens = 1.0
        else:
        ...
        if self.config.zoom and self.mousePosOnZoom is not None:
            mousePos = QtCore.QPointF(
                round((1 - sens) * self.mousePosOnZoom.x() +
                      (sens) * mousePosOrig.x()),
                round((1 - sens) * self.mousePosOnZoom.y() +
                      sens * mousePosOrig.y()))
        else:
            mousePos = mousePosOrig
        mousePosScaled = QtCore.QPointF(
            float(mousePos.x() - self.xoff) / self.scale,
    ...
    def getClosestPoint(self, poly, pt):
        ...
        if self.drawPolyClosed and poly.size() >= 2:
            for i in range(poly.size()):
                ...
                normalThroughMouse = QtCore.QLineF(pt.x(), pt.y(),
                                                   pt.x() + normal.dx(),
                                                   pt.y() + normal.dy())
                intersectionPt = QtCore.QPointF()
                intersectionType = edge.intersect(normalThroughMouse,
                                                  intersectionPt)
    ...
    def drawPoint(self, qp, pt, isFirst, increaseRadius):
        ...
        if isFirst:
            qp.setBrush(
                QtGui.QBrush(QtGui.QColor(0, 255, 0), QtCore.Qt.SolidPattern))
        # Other in red
        else:
            qp.setBrush(
    ...
    def appendObject(self, label, polygon):
        ...
        # Append to changes
        self.addChange("Created object {0} with label {1}".format(
            newID, label))
    ...
    def doScreenshot(self):
        ...
        if file:
            file = file.replace(
                '%i',
                os.path.splitext(os.path.basename(self.config.currentFile))[0])
    ...
    def blurLicensePlates(self, qp):
        ...
        for obj in self.annotation.objects:
            ...
            meanB = 0
            num = 0
            for y in range(max(int(bb.top()), 0),
                           min(int(bb.bottom() + 1.5), img.height())):
                for x in range(max(int(bb.left()), 0),
                               min(int(bb.right() + 1.5), img.width())):
                    col = img.pixel(x, y)
    ...
    def updateMouseObject(self):
        ...
        for idx in reversed(range(len(self.annotation.objects))):
            ...
            if obj.draw and self.getPolygon(obj).containsPoint(
                    self.mousePosScaled, QtCore.Qt.OddEvenFill):
                self.mouseObj = idx
                break
    ...
    def infoOnSelectedObject(self):
        ...
        if self.annotation and objID >= 0:
            ...
            self.statusBar().showMessage("Label of object {0}: {1}".format(
                obj.id, obj.label))
        # else:
        #    self.statusBar().showMessage(self.defaultStatusbar)
    ...
    def modifyLayer(self, offset):
        ...
        self.annotation.objects.insert(newidx,
                                       self.annotation.objects.pop(oldidx))
        
        # Update the selected object to the new index
        self.selObjs[-1] = newidx
        self.statusBar().showMessage(
            "Moved object {0} with label {1} to layer {2}".format(
                obj.id, obj.label, newidx))
        
        # Check if we moved the object the first time
        if not obj.id in self.changedLayer:
            self.changedLayer.append(obj.id)
            self.addChange(
                "Changed layer for object {0} with label {1}".format(
                    obj.id, obj.label))
